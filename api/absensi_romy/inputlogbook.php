...
$query = "SELECT COUNT(*) as jml FROM logbook WHERE tanggal ='$tgl' AND nik ='$nik' ";
$result = mysqli_query($KONEKSI,$query);
$data = mysqli_fetch_assoc($result);
$count = $data['jml'];

if($count > 0){
	$json = array('value'=> 0,'message'=> 'Pengajuan logbook di tanggal tersebut sudah ada ');
}else{
	$query = "INSERT INTO logbook (nik,tanggal,jam_mulai,jam_selesai,catatan,`status`)
          VALUES ('$nik','$tgl','$mulai','$selesai','$keterangan','0') ";
	...
	if($result){
        ...
        $query ="SELECT id FROM logbook  where nik ='$nik' order by id desc limit 1 ";
        $result = mysqli_query($KONEKSI, $query);
        $data   = mysqli_fetch_assoc($result);
        $id_logbook= $data['id'];
        ...
        // Notifikasi ke atasan
            $content = array(
                "en" => "$nama Sudah menginput kegiatan harian ",
            );
        ...
        $query ="INSERT INTO sys_pesan (id_flg_otorisasi,`user_id`,user_id_request,waktu,`subject`,header,pesan,fcm_token)
                    VALUES ('$id_logbook','$user_id_induk','$user_id',now(),'LOGBOOK','PENGAJUAN LOGBOOK','$nama Sudah menginput kegiatan harian','$fcm_token') ";
