...
class DetailLembur extends StatefulWidget {
  List list;
  int index;
  DetailLembur({this.index, this.list});
  @override
...
class _DetailLemburState extends State<DetailLembur> {
  ...
  approvalpermit() async {
    final response = await http.post(BaseUrl.approvallembur,
  ...
  rejectpermit() async {
    final response = await http.post(BaseUrl.rejectlembur,
        body: {"id": widget.list[widget.index]['id']});
    final data = jsonDecode(response.body);
    int value = data['value'];
    String pesan = data['message'];
              ...
              child: Column(
                children: [
                  ...
                  ListTile(
                    ...
                    subtitle:
                        new Text(widget.list[widget.index]['jam_awal'] ?? ''),
                  ),
                  Divider(),
                  ListTile(
                    title: new Text('selesai'),
                    subtitle:
                        new Text(widget.list[widget.index]['jam_akhir'] ?? ''),
                  ),
                  Divider(),
                  ListTile(
                    title: new Text('Selama'),
                    subtitle: new Text(
                        widget.list[widget.index]['selama'] + 'Jam' ?? ''),
                  ),
                  Divider(),
                  ListTile(
